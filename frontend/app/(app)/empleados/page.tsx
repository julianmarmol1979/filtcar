...
export default function EmpleadosPage() {
  ...
  useEffect(() => {
    const timer = setTimeout(() => fetchEmpleados(search), 300);
  ...
  async function handleToggle(e: Empleado) {
    ...
    if (!res.ok) {
      const err = await res.json().catch(() => ({}));
      alert(err.message ?? "No se puede cambiar el estado");
      return;
    }
    setEmpleados((prev) =>
      prev.map((x) => (x.id === e.id ? { ...x, activo: !x.activo } : x))
    );
  }
        ...
        {loading ? (
          ...
          <div className="py-16 flex flex-col items-center gap-2 text-gray-400">
            ...
            <p className="text-sm font-medium">
              {search ? "Sin resultados para la búsqueda" : "No hay empleados cargados"}
            </p>
          </div>
        ) : (
