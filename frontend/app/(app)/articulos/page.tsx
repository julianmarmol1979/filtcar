...
export default function ArticulosPage() {
  ...
  // Debounced search
  useEffect(() => {
    const timer = setTimeout(() => fetchArticulos(search), 300);
  ...
  async function handleToggle(a: Articulo) {
    ...
    setArticulos((prev) =>
      prev.map((x) => (x.id === a.id ? { ...x, activo: !x.activo } : x))
    );
  }
        ...
        {loading ? (
          ...
          <div className="py-16 flex flex-col items-center gap-2 text-gray-400">
            ...
            <p className="text-sm font-medium">
              {search ? "Sin resultados para la búsqueda" : "Todavía no hay artículos"}
            </p>
          </div>
        ) : (
            ...
            <form onSubmit={handleSave} className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                  placeholder="Ej: Aceite mineral 1L"
                />
              </div>
