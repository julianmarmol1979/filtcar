...
export default function CajaPage() {
  ...
  function openModal() {
    setForm({ tipo: "Ingreso", monto: 0, observacion: "" });
    setError("");
    setModalOpen(true);
  }
  ...
  const selectedTipo = tipoMeta(form.tipo);
  ...
  return (
    <div>
      ...
      <p className="text-xs text-gray-400 mt-2 text-right">{movimientos.length} movimientos</p>
                  ...
                  {TIPOS.map((t) => (
                    <button
                      ...
                      className={`py-1.5 rounded-lg text-xs font-semibold border-2 transition-all ${
                        form.tipo === t.value ? t.color + " border-transparent" : "border-gray-200 text-gray-600 hover:border-gray-300"
                      }`}
                    >
                      {t.label}
                    </button>
